...
```{r}
# Outliers in variables: 1,7,15,16,17,20,38
# Power Transform
m = lm(expected_survival ~ ., data = transformed_data)
powerTransform(m, family = "yjPower")
```
...
corr_matrix <- cor(robustSc_data, method = "spearman")
...
correlation_table <- corr_table(robustSc_data)
correlation_table
```
...
robust_model <- rlm(expected_survival ~ ., data = robustSc_data)
...
full_model <- lm(expected_survival ~ ., data = robustSc_data)
...
```{r}
# Robust PCA
robust_pca_data <- PcaHubert(scaled_trans_data, k=0, 
...
## To plot the results:
    ...
    pca2 <- PcaHubert(scaled_trans_data, k=2)  
...
```{r}
# Apply classical PCA using prcomp()
