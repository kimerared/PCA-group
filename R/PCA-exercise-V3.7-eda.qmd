...
library(rrcov)        # normal scores transformation
...
Fortunately, there are two outliers with no high leverage.
...
Normal score transformation, also known as z-score normalization, is a technique that transforms the data to have a mean of zero and a standard deviation of one. This transformation is useful for making data more comparable across different features or datasets.
...
-   The Pearson method was used because the features are near normally distributed.
...
correlation_table <- corr_table(transformed_data)
correlation_table
```
...
```{r}
qqnorm(scaled_df$expected_survival,
       ylab="Sample Quantiles for Expected Survival")

qqline(scaled_df$expected_survival, col="magenta")

plotNormalHistogram(scaled_df$expected_survival)
```

# 03 - Full Model Regression

-   The Adjusted R\^2 = 99.81% is an indication of over-fitting, or bias.
...
# Fit regression model
ordinary_model <- lm(expected_survival ~ ., data = scaled_df)
...
data_pca <- prcomp(scaled_df, center = TRUE, scale = TRUE)
summary(data_pca)
...
pc2_contribution +
  ...
  labs(title = "Contribution of Variables to PC1",
       ...
       x = "",
       caption = "PC1 explains 43.6% variance") +
  scale_y_continuous(labels = scales::percent_format(scale = 1,
                                                     accuracy = 1))


# Contributions of variables to PC2
pc2_contribution <- fviz_contrib(data_pca, choice = "var", axes = 2, top = 10)
...
get_target_index <- function(df, target_name){
  target_index <- grep(target_name, colnames(df))
  return(target_index)
}

training_target_index <- get_target_index(training_set, target_name)
test_target_index <- get_target_index(test_set, target_name)
```
...
```{r}
# Function to perform N-scores transformation to all variables except the dependent variable
nscores_transform <- function(data, dependent_col_name) {
  transformed_data <- data
  dependent_col <- transformed_data[, dependent_col_name]
  for (col in colnames(data)) {
    if (col != dependent_col_name) {
      transformed_data[, col] <- blom(data[, col])
    }
  }
  transformed_data[, dependent_col_name] <- dependent_col  # Keep the dependent variable as it is
  return(transformed_data)
}

# Perform the N-scores transformation on the training set, excluding the dependent variable
training_set <- nscores_transform(training_set, "expected_survival")

# Perform the N-scores transformation on the test set, excluding the dependent variable
test_set <- nscores_transform(test_set, "expected_survival")

# The transformed data now includes N-scores for all variables except the dependent variable
...
# pca_train <- prcomp(training_set_nscores, center = TRUE, 
#                     scale. = TRUE, rank. = 8)
# 
# # Apply the same PCA transformation to the test set
# pca_test <- predict(pca_train, newdata = test_set_nscores)
# 
# # Now, pca_train contains the PCA results for the training set, and pca_test contains the PCA results for the test set
...
# lm_model <- lm( ~ ., data = as.data.frame(pca_train$x))
# 
...
pca <- preProcess(training_set_nscores[, -training_target_index], 
                  method = 'pca', pcaComp = 8)

# Apply PCA transformation to original training set
training_set_pca <- predict(pca, training_set_nscores)

# Reorder columns, moving the dependent feature index to the end
training_set_pca <- training_set_pca[c(2:9, 1)]

# Apply PCA transformation to original test set
test_set_pca <- predict(pca, test_set_nscores)

# Reorder columns, moving the dependent feature index to the end
test_set_pca <- test_set_pca[c(2:9, 1)]
```
...
pca_full_model <- lm(expected_survival ~ ., data = training_set_pca)
...
corr_matrix <- cor(training_set_pca)
...
significant_pcs = c(1, 9)
train_pca <- training_set_pca[, significant_pcs]
test_pca <- test_set_pca[, significant_pcs]
```
...
# fit PCR
pcr_model <- pcr(y ~ PC1, data=train_pca, validation="CV")
