...
-   The loadings are the weights assigned to each variable for that particular principal component. The eigenvectors of the covariance or correlation matrix of the data represent the directions of maximum variance or information in the dataset.
...
# Variables that are part of the first two principal components
variables_first_two_components <- colnames(data_pca)[
  apply(abs(loadings_first_two_components) > 0.01, 2, any)]

# Print the variables
print("Variables that are part of the first two principal components:")
print(variables_first_two_components)
...
Top 10 variable contribution for the first two principal components.

```{r}
# Contributions of variables to PC1
pc1_contribution <- fviz_contrib(data_pca, choice = "var", axes = 1, top = 10)
# Modify the theme to rotate X-axis labels to 90 degrees
pc1_contribution + theme(
  axis.text.x = element_text(angle = 0),
  axis.text.y = element_text(angle = 0),
  plot.title = element_text(hjust = -4)# horizontal justification
  )+ coord_flip()+
  labs(title = "Contribution of Variables to PC1",
       caption = "(Percentage Contribution)")+
  xlab("")+
  ylab("Percentage")+
  scale_y_continuous(labels = scales::percent_format(scale = 10, accuracy = 1))

# Contributions of variables to PC2
pc2_contribution <- fviz_contrib(data_pca, choice = "var", axes = 2, top = 10)
# Modify the theme to rotate X-axis labels to 90 degrees
pc2_contribution + theme(
  axis.text.x = element_text(angle = 0),
  axis.text.y = element_text(angle = 0),
  plot.title = element_text(hjust = 7)# horizontal justification
  )+ coord_flip()+
  labs(title = "Contribution of Variables to PC2",
       x = "Percentage Contribution",
       y = "Variables Considered",
       caption = "(Percentage Contribution)")+
  xlab("Variables Considered")+
  ylab("Percentage")+
...
pc2_contribution +
  ...
  scale_y_continuous(labels = scales::percent_format(scale = 1,
                                                     accuracy = 1)) +
  scale_x_discrete(labels = c("Phosphorus > 7.0 Mg", "Phosphorus (3.5 - 4.5 Mg)", "Phosphorus < 3.5 Mg", "Phosphorus (5.6 - 7 Mg)", "Phosphorus (4.6 - 5.5 Mg)", "Worse Transplant Waitlist", "Fistula rate (B)", "Hgb 12 g", "Better Transplant Waitlist", "Better Survival"))
```
